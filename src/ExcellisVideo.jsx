...
const IntroScreen = () => {
  ...
  const particles = Array.from({ length: 12 }, (_, i) => ({ id: i, x: 20 + (i * 7) % 80, y: 10 + (i * 13) % 80, size: 2 + (i % 3), delay: i * 4 }));

  return (
    <AbsoluteFill style={{ background: NAVY, alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 60% 60% at 50% 50%, #1a3a6e 0%, " + NAVY + " 70%)" }} />
      {particles.map(p => {
        const pOpacity = interpolate(frame - p.delay, [0, 20], [0, 0.6], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
        return <div key={p.id} style={{ position: "absolute", left: p.x + "%", top: p.y + "%", width: p.size, height: p.size, borderRadius: "50%", background: GOLD, opacity: pOpacity }} />;
      })}
      <div style={{ opacity: logoOpacity, transform: "scale(" + interpolate(logoScale, [0, 1], [0.6, 1]) + ")", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ fontSize: 32, fontFamily: "Georgia, serif", color: WHITE, letterSpacing: "6px", fontWeight: "bold" }}>EXCELLIS</div>
        <div style={{ fontSize: 16, color: GOLD, letterSpacing: "8px", fontFamily: "Georgia, serif" }}>P R I V I L E G E</div>
      </div>
    </AbsoluteFill>
  );
};
...
const ServicesScreen = () => {
  ...
  return (
    <AbsoluteFill style={{ background: NAVY, opacity: bgOpacity, flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 0, padding: "0 60px" }}>
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 80% 50% at 50% 50%, #162d5e 0%, " + NAVY + " 80%)" }} />
      <div style={{ ...useFadeSlideUp(0, 20), color: GOLD, fontSize: 13, letterSpacing: "6px", fontFamily: "Georgia, serif", marginBottom: 20, zIndex: 1 }}>NOS SERVICES</div>
      {services.map((s) => {
        const anim = useFadeSlideUp(s.frame, 20);
        return (
          <div key={s.title} style={{ ...anim, zIndex: 1, display: "flex", alignItems: "center", gap: 20, width: "100%", maxWidth: 700, padding: "18px 0", borderBottom: "1px solid rgba(212,168,47,0.15)" }}>
...
const SloganScreen = () => {
  ...
  return (
    <AbsoluteFill style={{ background: NAVY, opacity: bgOpacity, alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 0 }}>
      ...
      <div style={{ ...textAnim, zIndex: 1, textAlign: "center", padding: "0 60px" }}>
        <div style={{ fontSize: 11, color: GOLD, letterSpacing: "5px", fontFamily: "Georgia, serif", marginBottom: 16 }}>NOTRE PROMESSE</div>
...
export const ExcellisVideo = () => (
  <AbsoluteFill>
    <Sequence from={0} durationInFrames={90}><IntroScreen /></Sequence>
    <Sequence from={90} durationInFrames={180}><ServicesScreen /></Sequence>
    <Sequence from={270} durationInFrames={180}><SloganScreen /></Sequence>
  </AbsoluteFill>
);
